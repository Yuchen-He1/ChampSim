...
METRICS = ["itlb_hit", "dtlb_hit", "stlb_hit", "stlb_miss", "l3tlb_hit", "l3tlb_miss"]  # from raw.*
RAW_FALLBACKS = {
    "stlb_miss": "stlb_ptw",
    "l3tlb_miss": "l3tlb_ptw",
}
...
def process_tlb_json_overlay(
    ...
    for m in METRICS:
        ...
        if raw_col in df.columns:
            ...
            fallback = RAW_FALLBACKS.get(m)
            if fallback:
                fallback_col = f"raw.{fallback}"
                if fallback_col in df.columns:
                    df[m] = df[fallback_col]
                    continue
            df[m] = 0
    ...
    agg["_sort_key"] = agg["dtlb_hit"] + agg["stlb_hit"] + agg["stlb_miss"] + agg["l3tlb_hit"] + agg["l3tlb_miss"]
